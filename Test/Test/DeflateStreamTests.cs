...
using Xunit;
using DeflateStream = AsyncDeflate.DeflateStream;

namespace Test
{
    public class DeflateStreamTests
    {
        static string gzTestFile(string fileName)
        {
        ...
        {
            CompressionLevel[] legalValues = new CompressionLevel[]
        ...
        public void CtorArgumentValidation()
        {
            ...
            Assert.Throws<ArgumentException>(
                () => new DeflateStream(new MemoryStream(new byte[1], writable: false), CompressionLevel.Optimal));
        }
        ...
        public async Task FlushAsyncFailsAfterDispose()
        {
            ...
            await Assert.ThrowsAsync<ObjectDisposedException>(async () => { await ds.FlushAsync(); });
        }
        ...
        public void TestSeekMethodsDecompress()
        {
            ...
            {
                long value = zip.Length;
            });
            Assert.Throws<NotSupportedException>(delegate
            {
                long value = zip.Position;
            });
        ...
        public void CopyToAsyncArgumentValidation()
        {
            using (DeflateStream ds = new DeflateStream(new MemoryStream(), CompressionMode.Decompress))
            {
                ...
                Assert.Throws<ArgumentOutOfRangeException>("bufferSize",
                    () => { ds.CopyToAsync(new MemoryStream(), 0); });
                Assert.Throws<NotSupportedException>(
                    () => { ds.CopyToAsync(new MemoryStream(new byte[1], writable: false)); });
                ds.Dispose();
                ...
                foreach (bool useAsync in new[] {true, false}) // whether to use Read/Write or ReadAsync/WriteAsync
                {
                    foreach (bool useGzip in new[] {true, false}) // whether to add on gzip headers/footers
                    {
                        foreach (
                                var level in
                                new[]
                                    {CompressionLevel.Fastest, CompressionLevel.Optimal, CompressionLevel.NoCompression}
                            )
                            // compression level
                        {
                            yield return new object[] {useAsync, useGzip, 1, 5, level}; // smallest possible writes
                            yield return new object[] {useAsync, useGzip, 1023, 1023 * 10, level};
                            // overflowing internal buffer
                            yield return new object[] {useAsync, useGzip, 1024 * 1024, 1024 * 1024, level};
                            // large single write
                        }
                    }
                }
        ...
        public async Task RoundtripCompressDecompress(bool useAsync, bool useGzip, int chunkSize, int totalSize,
            CompressionLevel level)
        {
            ...
            using (
                var compressor = useGzip
                    ? (Stream) new GZipStream(compressed, level, true)
                    : new DeflateStream(compressed, level, true))
            {
                for (int i = 0; i < data.Length; i += chunkSize)
                    // not using CopyTo{Async} due to optimizations in MemoryStream's implementation that avoid what we're trying to test
                {
                    switch (useAsync)
                    {
                        case true:
        ...
        [Theory]
        [MemberData(nameof(RoundtripCompressDecompressOuterData))]
        public async Task RoundTripWithFlush(bool useAsync, bool useGzip, int chunkSize, int totalSize,
            CompressionLevel level)
        {
            byte[] data = new byte[totalSize];
            new Random(42).NextBytes(data);
        ...
        [Theory]
        [MemberData(nameof(RoundtripCompressDecompressOuterData))]
        public async Task WriteAfterFlushing(bool useAsync, bool useGzip, int chunkSize, int totalSize,
            CompressionLevel level)
        {
            byte[] data = new byte[totalSize];
        ...
        [Theory]
        [MemberData(nameof(RoundtripCompressDecompressOuterData))]
        public async Task FlushBeforeFirstWrites(bool useAsync, bool useGzip, int chunkSize, int totalSize,
            CompressionLevel level)
        {
            byte[] data = new byte[totalSize];
            new Random(42).NextBytes(data);
            ...
            byte[] expected)
        {
            ...
            using (
                Stream decompressor = useGzip
                    ? (Stream) new GZipStream(compressed, CompressionMode.Decompress, true)
        ...
        {
            using (
                var ds = new DeflateStream(new BadWrappedStream(BadWrappedStream.Mode.ReturnNullTasks),
        ...
        {
            using (
                var ds = new DeflateStream(new BadWrappedStream(BadWrappedStream.Mode.ReturnTooLargeCounts),
                    CompressionMode.Decompress))
            ...
            using (
                var ds = new DeflateStream(new BadWrappedStream(BadWrappedStream.Mode.ReturnTooSmallCounts),
                    CompressionMode.Decompress))
                ...
                foreach (int copyBufferSize in new[] {1, 4096, 80 * 1024})
                {
                    ...
                    foreach (bool useAsync in new[] {true, false})
                    {
                        ...
                        yield return
                            ...
                            {
                                data, copyBufferSize,
                                new FileStream(path, FileMode.Open, FileAccess.Read, FileShare.Read, 0x1000, options)
                            };
                    }
                }
            }
        ...
        public async Task CopyToAsync_Roundtrip_OutputMatchesInput(byte[] expectedDecrypted, int copyBufferSize,
            Stream source)
        {
    ...
    public class ManualSyncMemoryStream : MemoryStream
    {
        ...
        public bool ReadHit = false;
        // For validation of the async methods we want to ensure they correctly delegate the async

        public bool WriteHit = false;
        // methods of the underlying stream. This bool acts as a toggle to check that they're being used.
    ...
    public abstract class DeflateStreamAsyncTestsBase
    {
        ...
        protected static string gzTestFile(string fileName)
        {
            return Path.Combine("GZTestData", fileName);
        }
        ...
        public async Task OverlappingWriteAsync()
        {
            ...
            using (var zip = new DeflateStream(writeStream, CompressionMode.Compress))
            {
                ...
                {
                    task = WriteAsync(zip, buffer, 0, buffer.Length);
                    // write needs to be bigger than the internal write buffer
                    Assert.True(writeStream.WriteHit);
                    Assert.Throws<InvalidOperationException>(() => { zip.WriteAsync(buffer, 32, 32); });
                    // "overlapping write"
                }
                finally
                {
        ...
        public async Task OverlappingReadAsync()
        {
            ...
            using (var unzip = new DeflateStream(readStream, CompressionMode.Decompress, true))
            {
                ...
                try
                {
                    ...
                    Assert.Throws<InvalidOperationException>(() => { ReadAsync(unzip, buffer, 0, 32); });
                    // "overlapping read"
                }
                finally
                {
